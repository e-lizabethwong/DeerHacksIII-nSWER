...
export function listCompletions(context, deploymentName, prompt, options = { requestOptions: {} }) {
    const { abortSignal, onResponse, requestOptions, tracingOptions } = options, rest = __rest(options, ["abortSignal", "onResponse", "requestOptions", "tracingOptions"]);
...
export function listChatCompletions(context, deploymentName, messages, options = { requestOptions: {} }) {
...
export async function getAudioTranslation(context, deploymentName, fileContent, formatOrOptions, inputOptions) {
    ...
    const { body, status } = await context
        ...
        tracingOptions,
        requestOptions,
    })), { contentType: "multipart/form-data", body: Object.assign(Object.assign({}, snakeCaseKeys(rest)), { file: createFile(fileContent, "placeholder.wav"), response_format }) }));
    if (status !== "200") {
        throw body.error;
    }
