...
function listCompletions(context, deploymentName, prompt, options = { requestOptions: {} }) {
    const { abortSignal, onResponse, requestOptions, tracingOptions } = options, rest = tslib.__rest(options, ["abortSignal", "onResponse", "requestOptions", "tracingOptions"]);
...
function listChatCompletions(context, deploymentName, messages, options = { requestOptions: {} }) {
...
async function getAudioTranslation(context, deploymentName, fileContent, formatOrOptions, inputOptions) {
    ...
    const { body, status } = await context
        ...
        tracingOptions,
        requestOptions,
    })), { contentType: "multipart/form-data", body: Object.assign(Object.assign({}, snakeCaseKeys(rest)), { file: coreRestPipeline.createFile(fileContent, "placeholder.wav"), response_format }) }));
    if (status !== "200") {
        throw body.error;
    }
